...
- 访问[这个链接](https://pan.zju.edu.cn/share/a52dced9508ae3cec3fa9d4093) 访问密码：zjueva，下载一个镜像。
- 启动镜像中包含的系统
- 使用用户`zjueva`和密码`zjueva`登录
- 在该用户的家目录下有个 OpenSSH 格式的私钥，私钥的口令是`zju3va`，使用这个私钥登录镜像中的Root用户
- 在`/root`中，找到`secret.txt`，将其中的内容和你在前面进行的操作写进文档，上传至答题仓库
- 在镜像的根目录下有个名为`secretfile`的文件，这是一个 LUKS 加密的 ext4 格式磁盘镜像，请尝试挂载这个镜像并读取里面的文件，将文件内容和你进行的操作上传至答题仓库。LUKS 口令为`170826`
